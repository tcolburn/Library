...
def get_sims(counts, method_names=[], exclude=[],
                 ext='.dcd', methods_basedir = 'paper_methods', topname=""):
...
def separate_sims(simulations, counts, exclude=[]):
    ...
    ** WORK IN PROGRESS **
    ...
    EX_10 = [simulations[i] for i in xrange(0,I) if i not in exclude]
    IM_25  = [simulations[i] for i in xrange(I,J) if i not in exclude]
    ABMD_10 = [simulations[i] for i in xrange(J,K) if i not in exclude]
    ABMD_25 = [simulations[i] for i in xrange(K,L) if i not in exclude]
    TMD_fast_10 = [simulations[i] for i in xrange(L,M) if i not in exclude]
    TMD_fast_25 = [simulations[i] for i in xrange(M,N) if i not in exclude]
...
def compact_labels(counts):
    ...
    ** WORK IN PROGRESS **
    ...
    ex_lab = ["*" for i in xrange(counts[0])]
    im_lab = ["~" for i in xrange(counts[1])]
    abmd_10_lab = ["_" for i in xrange(counts[2])]
    abmd_25_lab = ["__" for i in xrange(counts[3])]
    tmd_10_lab = ["+" for i in xrange(counts[4])]
    tmd_25_lab = ["++" for i in xrange(counts[5])]
...
def compute_timeseries(universe, frame_function, delta_t=.001, **kwargs):
    ...
    for i, ts in enumerate(universe.trajectory):
        ...
        print "Timestep: [" + str(i+1).zfill(3) + "/" + str(len(universe.trajectory)).zfill(3) + "] " + "\r",
...
def hdistance_calcs(psa_object):
    ...
    ** WORK IN PROGRESS **
    ...
    #         print "j = " + str(j)
            ...
            print "Sim #" + str(i + 1) + " [" + str(j+1) + "/" + str(numpaths) + "]" + "\r",
...
def ensemble_analysis_df(universe_list, frame_functions, func_names, traj_dir = "df_pkls/", delta_label = r"$\Delta \Delta RMSD$",  prefix="sim_", **kwargs):
    ...
    for i, universe in enumerate(universe_list):
        ...
        for j, function in enumerate(frame_functions):
            print "                                      Processing Trajectory " + traj_num + " using function [" + str(j+1) + "/" + str(len(frame_functions)) + "]" + "\r",
...
def gspace_plot(pickle_lists, bbox = (1.1, 0.1), cbar_loc=[0.5, 0.53, 0.03, 0.3],
    ...
    sub_plt_nums = [method_count*100 + 20 + (i+1) for i in xrange(method_count)]
...
def full_delta_plot(pickle_lists, gate_labels=gate_labels, df_labels=df_labels,
    ...
    sub_plt_nums = [method_num*100 + 10 + (i+1) for i in xrange(method_num)]
    t1, t2, t3, traj_len = gates_DIM_proc_adjusted(target, top)       # Target processing
    tgate_data = [t1, t2, t3]
